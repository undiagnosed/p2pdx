class User extends Class
	...
	getPath: (site=@hub) ->
		if site == Page.userdb
			return "merged-P2pdx/#{site}/data/userdb/#{@auth_address}"
		else
			return "merged-P2pdx/#{site}/data/users/#{@auth_address}"
	...
	getAvatarLink: ->
		...
		return "merged-P2pdx/#{@hub}/data/users/#{@auth_address}/avatar.#{@row.avatar}#{cache_invalidation}"
	...
	saveUserdb: (data, cb) ->
		...
		Page.cmd "fileGet", [@getPath(Page.userdb)+"/content.json", false], (userdb_data) =>
			...
			if changed
				@log "writing to userdb"
				@log @getPath(Page.userdb)+"/content.json"
				@log userdb_data
	...
	renderTopList: =>
		link = @getLink()
		h("a.list-group-item", {href: link, onclick: Page.handleLinkClick}, @row.user_name)
