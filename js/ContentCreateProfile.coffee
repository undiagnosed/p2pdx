            ...
            h("div.avatars", [
                hub.users.map (user) =>
                    if user.avatar not in ["jpg", "png"] or rendered >= 4
                        return ""
                    avatar = "merged-P2pdx/#{hub.address}/#{user.directory}/avatar.#{user.avatar}"
                    rendered += 1
    ...
    render: =>
        ...
        h("div#Content.center", [
            h("div.starter-template", [
                ...
                h("a.btn.btn-primary.btn-lg", {href: "#Select+user", onclick: @handleSelectUserClick}, [
                    h("span.glyphicon.glyphicon-user.fa-fw"),
